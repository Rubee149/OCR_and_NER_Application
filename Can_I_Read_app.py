...
import spacy
#spacy.cli.download("en_core_web_sm")   ##You need to download this model before launch this application
...
def get_text(Frame_output):
             ...
             for res in result:
                 st.write(HTML_WRAPPER.format(res[1]),unsafe_allow_html=True)
                 text += res[1] + " "
             st.write("**Extracted Complete text**")
...
def main():
    ...
    st.title("Can I Read?")
    st.sidebar.title("Created For Educational Purposes")
    ...
    st.sidebar.markdown("Basic Streamlit OCR & NER Application")
    st.sidebar.info("Build using Basic streamlit widgets and following packages ........ OpenCV-python, EasyOCR, SpaCy(en_core_web_sm NER model), Spacy-Streamlit ")

    st.sidebar.markdown("**By**")
    st.sidebar.markdown("_Rubee_")
